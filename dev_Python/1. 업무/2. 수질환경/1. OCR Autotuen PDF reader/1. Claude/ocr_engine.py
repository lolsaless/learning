...
# -*- coding: utf-8 -*-
"""
ocr_engine.py (gcms_autotune_ocr_eval.py 기반)
==========================================================================
...
# 3. 이미지 입력 / 전처리 유틸
# ==========================================================================
def load_page_image(path: str, dpi: int = 300, poppler_path: str = None) -> Image.Image:
    """PDF든 이미지든 받아서 첫 페이지를 PIL 이미지로 반환한다.

    poppler_path: poppler(pdftoppm) 실행파일이 있는 폴더 경로. None이면
    시스템 PATH에서 찾는다(배포용 실행파일은 번들된 경로를 넘겨준다).
    """
